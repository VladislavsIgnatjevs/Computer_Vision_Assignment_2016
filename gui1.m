...
% Last Modified by GUIDE v2.5 07-Nov-2016 20:48:02
...
%numPixels get, set and add to globals
numPixels = get(handles.numPixels,'String');
assignin('base','numPixels',numPixels)
set(handles.numPixels,'String',numPixels);

%throw error if empty directory path since path cannot be empty  
if isempty(InputPathName) && isempty(input_path_name)
    %err_msgs = 'Input folder path cannot be empty';
end
if isempty(output_folder_path)
    %msgbox('Input folder path cannot be empty', 'Error');
end    
...
switch choice
    case 'Select Images'
        ...
        [InputFileName,InputPathName,InputFilterIndex] = uigetfile({'*.jpg;*.tif;*.png;*.gif','All Image Files';...
         '*.*','All Files', },'MultiSelect','on');
     set(handles.imgPath,'String',InputPathName);
     
     %assign to global vars
     assignin('base','InputFileName',InputFileName);
     assignin('base','InputPathName',InputPathName);
