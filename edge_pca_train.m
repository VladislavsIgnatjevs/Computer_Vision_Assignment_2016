function X=edge_pca_train(url)

cd(url); %url of train set
n=8; % number of features we want to obtain
threshold=0.11;
...
coeff_edge=COEFF(:,1:n);
cd('../../../');
save('coeff_edge','coeff_edge');
X=temp_X*coeff_edge;
labels=[zeros(1,n1),ones(1,n2)];  %add label for trainning picture, 0 for manmade, 1 for natural
labels=labels.';
train_edge=[X,labels];   %feature matrix for classifying
save('train_edge','train_edge');